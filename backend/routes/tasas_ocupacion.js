...
const PYTHON_SCRIPT =
  process.env.ASINT_TASAS_SCRIPT ||
  path.join(BACKEND_ROOT, 'scripts', 'tasas_ocupacion_headless.py');
const RUNS_ROOT = path.join(BACKEND_ROOT, 'runs', 'tasas-ocupacion');
...
router.post('/run', upload.single('file'), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: 'Se requiere el archivo de entrada (campo "file").' });
  }
